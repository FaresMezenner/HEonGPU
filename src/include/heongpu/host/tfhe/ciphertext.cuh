...
namespace heongpu
{
    template <> class Ciphertext<Scheme::TFHE>
    {
      ...
      public:
         ...
         *
         * @param a_data Pointer to the LWE 'a' vector data (n * num_lwe
         * elements)
         * @param b_data Pointer to the LWE 'b' values (num_lwe elements)
         * @param num_lwe Number of LWE ciphertexts being set
         * @param stream CUDA stream for async operations
         */
        __host__ void set_lwe_data(const int32_t* a_data, const int32_t* b_data,
                                   int num_lwe,
         ...
         *
         * @param a_device Pointer to device LWE 'a' vector data (n * num_lwe
         * elements)
         ...
         */
        __host__ void
        set_lwe_data_device(const int32_t* a_device, const int32_t* b_device,
                            int num_lwe,
                            cudaStream_t stream = cudaStreamDefault)
        {
            shape_ = num_lwe;
            ...
            cudaMemcpyAsync(a_device_location_.data(), a_device,
                            a_size * sizeof(int32_t), cudaMemcpyDeviceToDevice,
                            stream);

            // Allocate and copy 'b' data (device to device)
            b_device_location_.resize(num_lwe, stream);
            cudaMemcpyAsync(b_device_location_.data(), b_device,
                            num_lwe * sizeof(int32_t), cudaMemcpyDeviceToDevice,
                            stream);

            // Initialize variances to a default value (required for TFHE ops)
            // R2L conversion introduces variance that can be approximated as
            // the key switching noise level (alpha_min)
            variances_.resize(num_lwe, alpha_min_);
         ...
         */
        __host__ void
        get_lwe_data(std::vector<int32_t>& a_out, std::vector<int32_t>& b_out,
